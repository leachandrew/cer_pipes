...
for(pipe in names){
  file_name<-paste("http://www.neb-one.gc.ca/open/energy/throughput-capacity/",pipe,"-throughput-and-capacity-dataset-eng.csv",sep="")
...
col_int<-c("Pipeline.Name","Key.Point","Product","Throughput","Actual_Capacity","Date")

Keystone_sub<-Keystone_data[,(names(Keystone_data) %in% col_int)]
TM_sub<-`Trans-Mountain_data`[,(names(`Trans-Mountain_data`) %in% col_int)]

`Enbridge-mainline_data`$Key.Point<-fct_collapse(`Enbridge-mainline_data`$Key.Point,
            "ex-Gretna" = c("Ex-Gretna", "ex-Gretna"))
`Enbridge-mainline_data`$Key.Point<-fct_collapse(`Enbridge-mainline_data`$Key.Point,
                                                 "into-Sarnia" = c("Into-Sarnia", "IntoSarnia"))
ENB_sub<-subset(`Enbridge-mainline_data`,Key.Point=="ex-Gretna")
ENB_sub<-subset(ENB_sub,Trade.Type!="Import")
ENB_sub<-ENB_sub[,(names(`Enbridge-mainline_data`) %in% col_int)]

test<-rbind(Keystone_sub,TM_sub,ENB_sub)
test$Product<-as.character(test$Product)
test$Product[test$Product=="domestic light"]<-"Domestic Light / NGL"
test$Product[test$Product=="domestic heavy"]<-"Domestic Heavy"
test$Product[test$Product=="refined petroleum products"]<-"Refined Petroleum Products"
test<-test %>% group_by(Pipeline.Name,Date) %>% mutate(Total_Throughput=sum(Throughput),cap_util=Total_Throughput/as.numeric(Actual_Capacity)) %>% ungroup()


all_mains<-test %>% group_by(Date,Product) %>% summarise(Throughput=sum(Throughput*6.2929)) %>% ungroup()
lims=c(Sys.Date()-years(10),max(test$Date)+months(3))


png<-1
if(png==1)#set these to only turn on if you're making PNG graphs
  set_png("all_pipe_products.png",height = 800)
ggplot(subset(all_mains,Date>=Sys.Date()-years(10)),aes(Date,Throughput,group = Product,colour=Product,fill=Product)) +
...
file_name<-"http://www.neb-one.gc.ca/open/energy/throughput-capacity/Apportionment-dataset-eng.csv"
...
df1$nominations_bbls<-as.numeric(as.character(df1$Total.Nominations..1000.m3.d.))*6.2929
df1$apportionment<-gsub("%","",df1$Apportionment.percent)
df1$apportionment<-as.numeric(as.character(df1$apportionment))
...
download.file("https://www.cer-rec.gc.ca/en/data-analysis/energy-commodities/crude-oil-petroleum-products/statistics/canadian-crude-oil-exports-rail-monthly-data.xlsx", destfile="cbr.xlsx", mode = "wb")

oil_by_rail<-read_excel("cbr.xlsx", sheet = NULL, range = NULL, col_names = TRUE,
...
pipe_capacity_data <- read.xlsx(xlsxFile = "pipes_capacity.xlsx", sheet = "10.7", startRow = 3,skipEmptyRows = TRUE,detectDates = TRUE)
...
df1<-melt(pipe_capacity_data,id=c("Year","CER.2020.Reference","CER.2020.Evolving","NEB.2016.Offtake","CAPP.2018.Offtake","CAPP.2014.Offtake","TM.Ref.Prod","Western.Refinery.Runs","Enbridge.Bakken.Volumes"), value.name = "Capacity",variable.name = "Pipeline")
...
makeColors <- function(){
  maxColors <- 11
  usedColors <- c()
  possibleColors <- colorRampPalette( brewer.pal( 11 , "PuOr" ) )(maxColors)
...
ggplot(df1) +
  ...
  geom_line(aes(Year,NEB.2016.Offtake*6.2929/1000, colour = "test" ),size=3) +
  ...
  scale_colour_manual("",labels=c("NEB 2016 Export Demand","CAPP 2018 Export Demand","CAPP 2014 Export Demand"),values=c("Black","Grey40","Firebrick"))+
  #scale_fill_viridis("",discrete=TRUE,option="D")+
...
df1<-melt(pipe_capacity_data,id=c("Year","CER.2020.Reference","CER.2020.Evolving","NEB.2016.Offtake","CAPP.2018.Offtake","CAPP.2014.Offtake"), value.name = "Capacity",variable.name = "Pipeline")
...
df1<-melt(pipe_capacity_data,id=c("Year","NEB.2016.Offtake","CAPP.2018.Offtake","CAPP.2014.Offtake"), value.name = "Capacity",variable.name = "Pipeline")
expansions<-c("Keystone.XL","Trans.Mountain.Expansion","Northern.Gateway","Energy.East")
...
if(png==1)#set these to only turn on if you're making PNG graphs
  set_png("pipe_capacity_real.png",height = 800,width = 1400)
ggplot(df1) +
  geom_area(data=filter(df1,df1$Pipeline!="Capacity Shortfall"),aes(Year,Capacity*6.2929/1000,fill=Pipeline),position="stack") +
  #geom_area(data=filter(df1,df1$Pipeline!="test"),aes(Year,Capacity*6.2929/1000,fill=Pipeline),position="stack",alpha=0.8) +
  geom_line(aes(Year,NEB.2016.Offtake*6.2929/1000, colour = "test" ),size=3) +
  ...
  #scale_color_viridis("",discrete=TRUE,option="D",labels="Expected Export Demand")+
  #scale_color_viridis("",discrete=TRUE,option="D",labels="Expected Export Demand")+
  scale_colour_manual("",labels=c("NEB (2016) Export Demand","Export Demand based on CAPP (2018) Forecast"),values=c("Black","Grey40"))+
  #scale_fill_viridis("",discrete=TRUE,option="D")+
  scale_fill_manual("",values = mkColor(df1$Pipeline),guide = "legend")+
