...
test$Product[test$Product=="foreign light"]<-"Foreign Light"
test$Product[test$Product=="domestic light / ngl"]<-"Domestic Light / NGL"
...
utilization <- test %>% group_by(Pipeline.Name,Date) %>% summarise(Throughput=sum(Throughput*6.2929),capacity=mean(as.numeric(Actual_Capacity)*6.2929),nameplate=mean(as.numeric(Nameplate_Capacity)*6.2929),
                                                                   utilization=Throughput/capacity)%>%
  ungroup() %>% group_by(Date) %>% mutate(total_util=sum(Throughput)/sum(capacity))%>% ungroup() %>%
  group_by(Pipeline.Name) %>% mutate(mean_util=rollapply(Throughput,12,mean,align='right',fill=NA)/rollapply(capacity,12,mean,align='right',fill=NA))



all_mains<-test %>% group_by(Date,Product) %>% summarise(Throughput=sum(Throughput*6.2929)) %>%
ungroup()
