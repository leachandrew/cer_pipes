...
labels_2 <- 
  ...
  mutate(area=as.character(area),
         area=gsub(" \\(","\n(",area),
         area=factor(area, levels = c("Alberta","Saskatchewan","US Midwest and Rockies\n(US PADDs 2 and 4)","US Gulf Coast\n(US PADD 3)")))
...
palette<-MetBrewer::met.brewer("Paquin", 10)
palette<-colors_tableau10()[-7]
palette<-viridis_pal(option="cividis",begin=0,end=.85)(4)

palette<-brewer.pal(8,"Greens")[5:8]

palette<-c("#9fba83","#8ab36e","#5a7c3e","#6b8c4f")

p2<-
    ggplot(mid_con_prod)+
    geom_text(
      data = labels_2,
      aes(x = date+months(2), y = prod/1000, label = area),colour="black",
      hjust = 0, size = 3, fontface = "bold", show.legend = FALSE
    ) +
    geom_area(aes(date,prod/1000,group=area,fill=area),linewidth=0.25,position = "stack",colour="black")+
...
ggsave("mid_continent_crude_prod.png",bg="white",width = 6,height=5,dpi=200)
